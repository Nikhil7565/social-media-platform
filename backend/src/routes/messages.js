...
router.get('/:otherUserId', authenticateToken, async (req, res) => {
    try {
        ...
        const msgs = await db.select()
            .from(messages)
            .where(or(and(eq(messages.senderId, userId), eq(messages.receiverId, otherUserId)), and(eq(messages.senderId, otherUserId), eq(messages.receiverId, userId))))
            .orderBy(messages.createdAt);
        // Fetch streak info
        const [streak] = await db.select().from(streaks)
            .where(or(and(eq(streaks.user1Id, userId), eq(streaks.user2Id, otherUserId)), and(eq(streaks.user1Id, otherUserId), eq(streaks.user2Id, userId))));
...
router.post('/:otherUserId', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const otherUserId = Number(req.params.otherUserId);
        ...
        // Handle Streak Logic
        let [streak] = await db.select().from(streaks).where(or(and(eq(streaks.user1Id, userId), eq(streaks.user2Id, otherUserId)), and(eq(streaks.user1Id, otherUserId), eq(streaks.user2Id, userId))));
        const now = new Date();
        if (!streak) {
