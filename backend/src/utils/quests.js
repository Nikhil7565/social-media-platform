...
export const updateQuestProgress = async (userId, type, subType) => {
    try {
        ...
        const userMissions = await db.select({
            ...
            .where(and(eq(userQuests.userId, userId), eq(userQuests.date, date), eq(quests.type, type), subType ? eq(quests.id, subType) : sql `1=1`, // If subtype provided, match specific quest ID
        ...
        for (const mission of userMissions) {
            ...
            await db.update(userQuests)
                ...
                .where(and(eq(userQuests.userId, userId), eq(userQuests.questId, mission.id), eq(userQuests.date, date)));
        }
    }
    catch (error) {
